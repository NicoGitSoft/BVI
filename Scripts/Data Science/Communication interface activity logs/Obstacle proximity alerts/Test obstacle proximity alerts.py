...
for i in range(N):
    if buzzer_messages[i] == 1:
        ...
        plt.text(times[i], 1.5, r"$\eighthnote$", color='k', fontsize=16, label=r'Detection of an obstacle $\eighthnote$')

plt.plot(times, z, color='r', label=r'Measured distance to the test obstacle')
plt.axhline(y=1, color='k', linestyle="--" , linewidth=1, alpha=0.5)
plt.axhline(y=2, color='k', linestyle="--" , linewidth=1, alpha=0.5, label=r'Upper and lower threshold for alerts')

#plt.plot(times, d, label=r'detected')
#plt.axhline(y=DELTA, color='k', linestyle="--" , linewidth=1, alpha=0.5, label=r'Umbral de detección')
plt.plot([], [], ' ',color='k', label=r'Alert of an obstacle detected $\eighthnote$')
#plt.title(r"Obstacle proximity alerts")
plt.xlabel(r'Time [s]')
plt.ylabel(r'Distance [m]')
plt.legend(loc='upper right')
plt.xlim(times[0], times[-1])
plt.savefig('Test obstacle proximity alerts.svg', format='svg', dpi=1200, bbox_inches='tight')
plt.savefig('Test obstacle proximity alerts.png', format='png', dpi=1200, bbox_inches='tight')
plt.savefig('Test obstacle proximity alerts.pdf', format='pdf', dpi=1200, bbox_inches='tight')
plt.show()