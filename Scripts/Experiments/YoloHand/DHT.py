...
import board, adafruit_dht, subprocess
...
DHT22Temperature = []  # Muestras de la temperatura del sensor DHT22
...
while True:
    ...
    if (current_time - frames_timer) > 1:
        ...
        try:
            DHT22Temperature.append(dhtDevice.temperature) # Temperatura del sensor DHT22
        except:
            DHT22Temperature.append(0.0)
            continue
        ...
        print(
            ...
            "DHT22 temperature: {:.2f} °C".format(DHT22Temperature[-1]),
            sep = "\t"#, end = "\r"
        )
...
with open('data.csv', 'w', newline='') as file:
    ...
    writer.writerow(["times", "z", "d", "h", "v", "nearest_labels", "haptic_messages", "buzzer_messages", "chipTemperature", "cpuTemperature", "DHT22Temperature"])
    writer.writerows(zip(times, z, d, h, v, nearest_labels, haptic_messages, buzzer_messages, chipTemperature, cpuTemperature, DHT22Temperature))