...
deltaX, deltaY = 0.045, .07
ax = fig.add_axes([deltaX, deltaY, 1 - 2 * deltaX, 1 - 2 * deltaY])
...
plt.plot(data.times, data.h, label=r'Distancia horizontal')
plt.plot(data.times, data.v, label=r'Distancia vertical')
plt.axhline(y=DELTA, color='k', linestyle="--" , linewidth=1, alpha=0.5, label=r'Umbral de detección')
plt.plot([], [], ' ',color='k', label=r'$\twonotes$ Infotmar detección')
plt.title(r"Posicion de la detección más cercana al centro de la imagen", size=20)
plt.xlabel(r'Tiempo [s]')
plt.ylabel(r'Distancia en píxeles [px]')
plt.savefig('exp2.svg', format='svg', dpi=1200)
plt.savefig('exp2.png', format='png', dpi=1200)
plt.savefig('exp2.pdf', format='pdf', dpi=1200)
plt.show()