import serial, cv2, os, time, json, math, csv
import numpy as np
...
try:
    serial = serial.Serial("/dev/ttyS0", 9600, timeout=1) # Establecer un objeto para comunicaciøn unidireciønal serial en el pin Rx 
...
while True:
    ...
    #depth_filtered_frame = cv2.applyColorMap(FilterDepth(depthFrame, zmin, zmax), cv2.COLORMAP_AUTUMN)
    ...
    #print(
    #    "sample: " + str(len(times)),
    #    "t: {:.2f} [s]".format(times[-1]),
    #    "fps: {:.2f}".format(fps),
    #    "z: {:.2f} [m]".format(z[-1]),
    #    "class: " + str(nearest_labels[-1]) + " d: {:.2f} [m]".format(d[-1]) + " msg: " + haptic_messages[-1] if d[-1] != "nan" else
    #    "class: " + nearest_labels[-1] + " d: " + d[-1] + " msg: " + haptic_messages[-1],
    #    end="\n", sep=" ")
    ...
    #        os.makedirs("images")
    #    cv2.imwrite("images/depthFrame_"+str(len(times))+".png", depthFrameColor)
    #    cv2.imwrite("images/depth_filtered_frame_"+str(len(times))+".png", depth_filtered_frame)
    #    cv2.imwrite("images/rgbFrame_"+str(len(times))+".png", frame)
